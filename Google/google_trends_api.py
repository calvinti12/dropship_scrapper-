...
class GoogleTrendsApi:
    ...
    def create_tasks_func(self, kw_list, hours_in_trend):
        with futures.ThreadPoolExecutor(max_workers=5) as executor:
            tasks = [
                executor.submit(self.is_trend, kw_list, 'now 7-d', '', 5, hours_in_trend),
                executor.submit(self.is_trend, kw_list, 'now 7-d', 'US', 5, hours_in_trend),
                executor.submit(self.is_trend, kw_list, 'now 7-d', 'CA', 5, hours_in_trend),
                executor.submit(self.is_trend, kw_list, 'now 7-d', 'GB', 5, hours_in_trend),
                executor.submit(self.is_trend, kw_list, 'now 7-d', 'AU', 5, hours_in_trend)
            ]
    ...
    def get_interest_over_time(self, kw_list, hours_in_trend, debug=False):
        ...
        if debug:
            result = self.is_trend(kw_list, 'now 7-d', 'CA', 5, hours_in_trend, debug=debug)
        else:
